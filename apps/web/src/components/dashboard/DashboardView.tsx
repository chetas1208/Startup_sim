...
import CompetitorDetail from '@/components/dashboard/CompetitorDetail';
import MarketSizing from '@/components/dashboard/MarketSizing';
import InvestorLandscape from '@/components/dashboard/InvestorLandscape';
import DemandSignals from '@/components/dashboard/DemandSignals';
import WhiteSpace from '@/components/dashboard/WhiteSpace';
...
import { sortCompetitors } from '@/lib/helpers';
import type { DashboardData, Competitor } from '@/lib/types';
...
export default function DashboardView({ data, onNewSearch }: Props) {
    ...
    return (
        <div className="min-h-screen bg-sg-bg">
                    ...
                    <div className="flex items-center justify-between h-14">
                        <div className="flex items-center gap-3 min-w-0">
                            <h1 className="text-sm font-bold text-sg-accent whitespace-nowrap">
                                Venture<span className="text-sg-text">Forge</span>
                            </h1>
                            <span className="text-sg-border">|</span>
                            <p className="text-sm text-sg-textMuted truncate">{data.query}</p>
                        </div>
                        <button onClick={onNewSearch} className="btn-ghost text-xs flex items-center gap-1.5 flex-shrink-0">
                ...
                <section ref={(el) => { sectionRefs.current.landscape = el; }} id="landscape">
                    ...
                    <div className="mt-8">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-base font-medium text-sg-text">{data.competitors?.length} Competitors Found</h3>
                            <div className="relative">
                                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
                                    className="appearance-none bg-sg-surface border border-sg-border rounded-lg px-3 py-1.5 pr-8 text-xs text-sg-textMuted focus:outline-none focus:border-sg-borderLight cursor-pointer">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {sorted.map((comp) => (
                                <CompetitorCard key={comp.id || comp.name} competitor={comp} onClick={setSelectedCompetitor} />
                            ))}
                        </div>
                    </div>
                </section>
            ...
            <button onClick={() => setShowBriefing(true)}
                className="fixed bottom-6 right-6 z-30 flex items-center gap-2 px-5 py-3 bg-sg-accent text-sg-bg font-semibold rounded-full shadow-lg shadow-sg-accent/25 hover:bg-sg-accentDim transition-all duration-200 hover:shadow-xl hover:shadow-sg-accent/30">
                <Volume2 className="w-4 h-4" /> Brief Me
            </button>

            {/* Scroll to Top */}
            <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
                className="fixed bottom-6 left-6 z-30 w-10 h-10 rounded-full bg-sg-surface border border-sg-border text-sg-textMuted hover:text-sg-text hover:border-sg-borderLight flex items-center justify-center transition-colors">
                <ArrowUp className="w-4 h-4" />
            </button>

            {/* Voice Briefing Modal */}
            {showBriefing && <VoiceBriefing text={data.voice_briefing_text} onClose={() => setShowBriefing(false)} />}

            {/* Competitor Detail Panel */}
            {selectedCompetitor && <CompetitorDetail competitor={selectedCompetitor} onClose={() => setSelectedCompetitor(null)} />}
        </div>
    );
}
