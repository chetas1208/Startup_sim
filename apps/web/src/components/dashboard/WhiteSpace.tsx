...
import type { WhiteSpaceGap } from '@/lib/types';
...
interface Props { gaps: WhiteSpaceGap[] | undefined; }

export default function WhiteSpace({ gaps }: Props) {
                        ...
                        <div key={gap.id || i} className="glass-panel p-5">
                            <div className="flex items-start justify-between gap-4 mb-3">
                                <div className="flex items-start gap-3">
                                    <div className="mt-0.5 w-8 h-8 rounded-lg bg-sg-yellow/10 border border-sg-yellow/20 flex items-center justify-center flex-shrink-0">
                                        <Lightbulb className="w-4 h-4 text-sg-yellow" />
                                    </div>
                                    <h3 className="text-sm font-semibold text-sg-text leading-snug">{gap.gap}</h3>
                                </div>
                                <span className={`text-xs px-2 py-0.5 rounded border flex-shrink-0 ${sizeConfig.color}`}>{sizeConfig.label} opportunity</span>
                            </div>
                            <p className="text-sm text-sg-textMuted leading-relaxed ml-11">{gap.evidence}</p>
                            {gap.competitors_missing && gap.competitors_missing.length > 0 && (
                                <div className="mt-3 ml-11 flex items-center gap-2 flex-wrap">
                                    ...
                                    {gap.competitors_missing.map((c, j) => (
                                        <span key={j} className="text-xs px-2 py-0.5 rounded bg-sg-surfaceLight text-sg-textMuted border border-sg-border">{c}</span>
                                    ))}
                                </div>
                            )}
                        </div>
