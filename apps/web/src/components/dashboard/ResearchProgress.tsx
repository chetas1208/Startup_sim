...
export default function ResearchProgress({ query, onComplete }: Props) {
    const [visibleSteps, setVisibleSteps] = useState<typeof PROGRESS_STEPS>([]);
        ...
        <div className="min-h-screen flex flex-col items-center justify-center px-4">
            <div className="w-full max-w-xl">
                <div className="mb-10 text-center animate-fade-in">
                    <p className="text-sm text-sg-textDim uppercase tracking-wider font-medium mb-2">Researching</p>
                    <p className="text-lg text-sg-text font-medium leading-relaxed">&ldquo;{query}&rdquo;</p>
                </div>
                ...
                <div className="flex justify-center mb-10">
                    <div className="relative w-16 h-16">
                        ...
                        <div className={`absolute inset-3 rounded-full flex items-center justify-center transition-all duration-500 ${done ? 'bg-sg-green scale-110' : 'bg-sg-accent'
                            }`}>
                            {done ? (
                ...
                <div className="space-y-3">
                  ...
                  ${isComplete ? 'bg-sg-green/10 border border-sg-green/20' : ''}`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className={`text-sm ${isComplete ? 'text-sg-green font-semibold' : 'text-sg-text'}`}>{step.text}</p>
                                    {step.source && <p className="text-xs text-sg-textDim mt-0.5 font-mono">via {step.source}</p>}
                                </div>
                ...
                {done && (
                    <div className="mt-8 animate-slide-up flex justify-center">
                        <button onClick={onComplete} className="btn-primary text-base px-8 py-3">View Results →</button>
                    </div>
                )}
            </div>
        </div>
