...
export default function VoiceBriefing({ text, onClose }) {
      ...
      <div className="relative w-full max-w-2xl glass-panel p-6 animate-slide-up">
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-sg-accent/10 border border-sg-accent/20
        ...
        <div className="h-1 bg-sg-surface rounded-full mb-5 overflow-hidden">
          <div
            className="h-full bg-sg-accent rounded-full transition-all duration-200"
        ...
        <div className="flex items-center justify-center gap-4 mb-6">
          <button
            onClick={restart}
            className="p-2 rounded-full hover:bg-sg-surface text-sg-textMuted hover:text-sg-text transition-colors"
          >
            <RotateCcw className="w-5 h-5" />
          </button>
          <button
            onClick={togglePlayback}
            className="w-14 h-14 rounded-full bg-sg-accent flex items-center justify-center
                       hover:bg-sg-accentDim transition-colors"
          >
            {playing ? (
              <Pause className="w-6 h-6 text-sg-bg" />
            ) : (
              <Play className="w-6 h-6 text-sg-bg ml-0.5" />
            )}
          </button>
          <div className="w-9" />
        </div>
