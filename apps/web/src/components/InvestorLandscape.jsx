...
export default function InvestorLandscape({ investors, competitors }) {
  if (!investors) return null;

  const fundingTimeline = competitors
    ?.flatMap((c) =>
      (c.funding_rounds || []).map((r) => ({
        company: c.name,
        ...r,
      }))
    )
  ...
  const investorOverlap = investors.filter(
    (inv) => inv.companies_in_space.length > 1
  );
  ...
  return (
    <div className="space-y-6">
          ...
          <div key={inv.id} className="glass-panel-hover p-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {inv.companies_in_space.map((c, i) => (
                <span
                  key={i}
                  className="text-xs px-2 py-0.5 rounded bg-sg-purple/10 text-sg-purple border border-sg-purple/20"
                >
                  {c}
                </span>
              ))}
      ...
      {investorOverlap.length > 0 && (
        <div>
          <h3 className="text-sm font-medium text-sg-textDim uppercase tracking-wider mb-3">
          ...
          <div className="glass-panel p-4 space-y-3">
            {investorOverlap.map((inv) => (
              <div key={inv.id} className="flex items-center gap-3 text-sm">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {inv.companies_in_space.map((c, i) => (
                    <span key={i} className="text-xs px-2 py-0.5 rounded bg-sg-surfaceLight text-sg-textMuted">
                      {c}
                    </span>
                  ))}
                </div>
