...
import { CLASSIFICATION_CONFIG } from '../utils/helpers';
...
function VideoSection({ videos }) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {videos.map((video, i) => {
          const thumbUrl = video.thumbnail_id
            ? `https://img.youtube.com/vi/${video.thumbnail_id}/mqdefault.jpg`
            : null;
          const isPlaying = playingId === i;

          if (isPlaying) {
            const embedId = video.thumbnail_id || '';
            return (
              <div key={i} className="col-span-2 rounded-lg overflow-hidden border border-sg-border bg-black">
                ...
                <div className="px-3 py-2 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-xs text-sg-textDim hover:text-sg-text ml-2 flex-shrink-0"
                  >
                    Close
                  </button>
                </div>
          ...
          return (
            <button
              ...
              className="group rounded-lg overflow-hidden border border-sg-border
                         hover:border-sg-borderLight transition-all text-left"
            >
              {/* Thumbnail */}
              <div className="relative w-full bg-sg-surfaceLight" style={{ paddingBottom: '56.25%' }}>
                {thumbUrl ? (
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-sg-surface">
                    <Video className="w-8 h-8 text-sg-textDim" />
                  </div>
                )}
                {/* Play overlay */}
                <div className="absolute inset-0 bg-black/30 flex items-center justify-center
                                opacity-0 group-hover:opacity-100 transition-opacity">
                  <div className="w-10 h-10 rounded-full bg-sg-accent/90 flex items-center justify-center">
                    <Play className="w-4 h-4 text-sg-bg ml-0.5" />
                  </div>
                </div>
                {/* Duration badge */}
                <span className="absolute bottom-1.5 right-1.5 text-[10px] px-1.5 py-0.5 rounded
                                 bg-black/80 text-white font-mono">
                  {video.duration}
                </span>
              </div>
              {/* Info */}
              <div className="px-2.5 py-2">
                <p className="text-xs text-sg-text leading-snug line-clamp-2 group-hover:text-sg-accent transition-colors">
                  {video.title}
                </p>
      ...
      <p className="text-[10px] text-sg-textDim mt-2 italic">
        Analyzed by Modulate · Click to preview
      </p>
    </div>
  );
}

export default function CompetitorDetail({ competitor, onClose }) {
      ...
      <div className="relative w-full max-w-lg bg-sg-bg border-l border-sg-border overflow-y-auto animate-slide-in-right">
        ...
        <div className="sticky top-0 z-10 bg-sg-bg/95 backdrop-blur border-b border-sg-border px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-lg hover:bg-sg-surface text-sg-textMuted hover:text-sg-text transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="px-6 py-5 space-y-6">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {competitor.features?.map((f, i) => (
                <span key={i} className="text-xs px-2.5 py-1 rounded-lg bg-sg-surfaceLight border border-sg-border text-sg-textMuted">
                  {f}
                </span>
              ))}
            </div>
          ...
          {/* Tech Stack */}
          {competitor.technologies?.length > 0 && (
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {competitor.technologies.map((t, i) => (
                  <span key={i} className="text-xs px-2.5 py-1 rounded-lg bg-sg-orange/10 border border-sg-orange/20 text-sg-orange">
                    {t}
                  </span>
                ))}
              </div>
              ...
              <div className="space-y-2">
                {competitor.people.map((p, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-sg-surfaceLight border border-sg-border
                                    flex items-center justify-center text-xs font-medium text-sg-textMuted">
          ...
          {/* Funding History */}
          {competitor.funding_rounds?.length > 0 && (
            <div>
                  ...
                  <div key={i} className="relative pl-6 pb-3 border-l border-sg-border last:border-l-0 last:pb-0">
                    ...
                    <div className="flex flex-wrap gap-1 mt-1">
                      {round.investors?.map((inv, j) => (
                        <span key={j} className="text-xs text-sg-purple">{inv}{j < round.investors.length - 1 ? ',' : ''}</span>
                      ))}
                    </div>
                  </div>
                ))}
