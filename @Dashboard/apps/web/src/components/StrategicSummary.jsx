...
export default function StrategicSummary({ summary }) {
  if (!summary) return null;

  const attractiveConfig = {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="glass-panel p-5">
          <h3 className="text-sm font-medium text-sg-textDim uppercase tracking-wider mb-3 flex items-center gap-1.5">
            <CheckCircle className="w-3.5 h-3.5 text-sg-accent" />
          ...
          <ul className="space-y-2.5">
            {summary.must_be_true?.map((item, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-sg-textMuted">
                <span className="text-sg-accent font-mono text-xs mt-0.5">{i + 1}.</span>
                {item}
              </li>
            ))}
          </ul>
        ...
        <div className="glass-panel p-5">
          <h3 className="text-sm font-medium text-sg-textDim uppercase tracking-wider mb-3 flex items-center gap-1.5">
            <AlertTriangle className="w-3.5 h-3.5 text-sg-red" />
            Biggest Risks
          </h3>
          <ul className="space-y-2.5">
            {summary.risks?.map((item, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-sg-textMuted">
                <span className="text-sg-red mt-1.5 flex-shrink-0">•</span>
                {item}
              </li>
            ))}
          </ul>
      ...
      <div className="glass-panel p-5 border-sg-accent/20 bg-sg-accent/5">
        <h3 className="text-sm font-medium text-sg-textDim uppercase tracking-wider mb-3 flex items-center gap-1.5">
          <ArrowRight className="w-3.5 h-3.5 text-sg-accent" />
          Recommended Next Steps
        </h3>
        <ul className="space-y-3">
          {summary.next_steps?.map((item, i) => (
            <li key={i} className="flex items-start gap-3 text-sm">
              <span className="flex-shrink-0 w-6 h-6 rounded-full bg-sg-accent/20 text-sg-accent
