  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        {themes.map((theme) => (
          <span key={theme} className="text-xs px-2.5 py-1 rounded-full bg-sg-surfaceLight border border-sg-border text-sg-textMuted">
            {theme}
          </span>
        ))}
      </div>
            ...
            <div key={signal.id} className="glass-panel p-4">
              <div className="flex items-start justify-between gap-3 mb-2">
                <div className="flex items-center gap-2">
                  ...
                  {signal.subreddit && (
                    <span className="text-xs text-sg-textDim font-mono">{signal.subreddit}</span>
                  )}
              ...
              <div className="flex gap-3">
                ...
                <p className="text-sm text-sg-textMuted leading-relaxed italic">"{signal.text}"</p>
              </div>
              <div className="mt-2 ml-7">
                <span className="text-xs px-2 py-0.5 rounded bg-sg-surfaceLight text-sg-textDim border border-sg-border">
                  {signal.theme}
                </span>
              </div>
            </div>
