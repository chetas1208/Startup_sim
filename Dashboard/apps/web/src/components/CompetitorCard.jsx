...
export default function CompetitorCard({ competitor, onClick }) {
    ...
    >
      ...
      <div className="flex flex-wrap gap-1.5">
        {competitor.features?.slice(0, 3).map((feature, i) => (
          <span
            key={i}
            className="text-xs px-2 py-0.5 rounded bg-sg-surfaceLight text-sg-textMuted
        ...
        {competitor.features?.length > 3 && (
          <span className="text-xs px-2 py-0.5 text-sg-textDim">
            +{competitor.features.length - 3}
          </span>
        )}
      </div>
